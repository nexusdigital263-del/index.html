...
function recentActivity(leads, limit = 8) {
  ...
  leads.forEach((l) => {
    (l.interacoes || []).forEach((it) => {
      if (!it || it.kind || !it.data || !it.tipo) return; // ignora marcadores de sistema (lid/optout) e itens incompletos
...
// lead já recebeu pelo menos uma mensagem de WhatsApp ENVIADA (1º contato feito).
// Serve para saber quem ainda falta prospectar ao retomar a campanha.
function leadProspected(lead) {
  return !!(lead && (lead.interacoes || []).some((it) => it.tipo === "WhatsApp" && it.dir !== "in"));
}
...
Object.assign(window, {
  ...
  leadOptedOut, leadProspected, prospeccaoMetrics,
