...
(function () {
  ...
  // ---- BUILT-IN connection -------------------------------------------------
  //  Preencha aqui para que TODOS já abram conectados, sem configurar nada.
  //  A chave anon/publishable é segura no navegador (as permissões RLS
  //  protegem os dados no servidor). Deixe url vazio para voltar ao modo
  //  "cada um conecta" / demonstração.
  const BUILTIN_CFG = {
    url: "https://amimshmlwzlnhluqltcf.supabase.co",
    anonKey: "sb_publishable_Oz8QSw5fuQUFiOYvzuUvRQ_pb0HHQXY",
  };
  const hasBuiltin = !!(BUILTIN_CFG.url && BUILTIN_CFG.anonKey);

  // ---- config persistence --------------------------------------------------
  function getCfg() {
    if (hasBuiltin) return BUILTIN_CFG;
  ...
  function isBuiltin() { return hasBuiltin; }
